...
from config.config import dp, bot, chinese, menu, choice_zodiac, zodiak_name_list, zodiac_date_list, my_zodiac_year, space_list
...
class Japanese(StatesGroup):
    zodiac_choice = State()
    birtday_choice = State()
    zodiac_name = State()
    pagination_japan_page = State()


@dp.callback_query_handler(text=["Японский 🈴"])
async def japanese_inline_keyboard(call: CallbackQuery):
    """Колбек-хендлер который ловит кнопку Японский 🈴"""
    await Japanese.zodiac_choice.set()
...
@dp.message_handler(lambda message: message.text, state=Japanese.zodiac_choice)
async def check_birthday(message: Message, state: FSMContext):
    ...
    if message.text == "/start":
        ...
        await message.reply("Воспользуйтесь кнопками📱")


@dp.callback_query_handler(state=Japanese.zodiac_choice, text=["Определите Мой Знак Зодиака ❓"])
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=Japanese.birtday_choice)
async def check_birthday(message: Message, state: FSMContext):
    """Хендлер который проверяет являются ли введенные данные числом"""
    if message.text == "/start":
        await state.finish()
...
@dp.message_handler(lambda message: not 3 < len(message.text) < 5 and message.text.isdigit() or not 1935 < int(message.text) < 2024 and message.text.isdigit(), state=Japanese.birtday_choice)
async def check_birthday(message: Message, state: FSMContext):
...
@dp.message_handler(lambda message: 3 < len(message.text) < 5 and message.text.isdigit() and 1935 < int(message.text) < 2024, state=Japanese.birtday_choice)
async def check_birthday(message: Message, state: FSMContext):
...
@dp.callback_query_handler(state=Japanese.zodiac_choice, text=["Я знаю Свой Знак Зодиака 💯"])
async def i_know_my_zodiac_sign(call: CallbackQuery, state: FSMContext):
    ...
    await state.finish()
    await Japanese.zodiac_name.set()
    await bot.delete_message(chat_id=call.from_user.id, message_id=call.message.message_id)
    await call.message.answer("Выберите тотемное животное 🕉", reply_markup=keyboard(3, chinese, 12, space_list()))


@dp.message_handler(lambda message: message.text, state=Japanese.zodiac_name)
async def check_birthday(message: Message, state: FSMContext):
...
@dp.callback_query_handler(state=Japanese.zodiac_name, text=[key for key in chinese().keys()])
async def date_inline_keyboard(call: CallbackQuery, state: FSMContext):
    ...
    await send_page(return_history(f'{user_data["Тотемное животное"]}_japanese')[0][0].split("\n"), call.message)
    await Japanese.next()
    await call.answer()


@dp.message_handler(lambda message: message.text, state=Japanese.pagination_japan_page)
async def check_birthday(message: Message, state: FSMContext):
    """Хендлер который отклоняет введение с клавиатуры"""
    if message.text == "/start":
        await state.finish()
...
@dp.callback_query_handler(lambda call: True, state=Japanese.pagination_japan_page)
